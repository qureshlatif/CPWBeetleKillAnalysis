...
dat.plt <- dat.supp %>% filter(stratum == "LP")
...
# Percent dead X Years since outbreak #
...
## Percent dead, spruce-fir ##
dat.plt <- dat.supp %>% filter(stratum == "SF")
...
# Percent dead X Years since outbreak #
p.pdXYSO.SF <- ggplot(dat = dat.plt, aes(x = index, y = bb.pdXYSO)) +
  geom_errorbar(aes(ymin = bb.pdXYSO.lo, ymax = bb.pdXYSO.hi), size=1, width=0) +
  geom_point(size = 2.5) + 
  geom_hline(yintercept = 0) +
  coord_flip() +
...
save_plot("Plot_outbreak_effects_SuppSpp.tiff", p, ncol = 4, nrow = 4.5, dpi = 200)
